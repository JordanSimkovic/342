...
rand_row_ind = sample(c(1:50), size = 50, replace = FALSE)
rand_col_ind = sample(c(1:50), size = 50, replace = FALSE)
mat = matrix(0, nrow = 50, ncol = 50)
for (i in rand_row_ind) {
  for (j in rand_col_ind){
    if (num_zero < 1250) {
      mat[i, j] = 0
      num_zero = num_zero + 1
    } else if (num_one < 625) {
...
```{r}
hole_punch_row_vec = sample(c(1:50), size = 50, replace = FALSE)
hole_punch_col_vec = sample(c(1:50), size = 50, replace = FALSE)

num_hole_punch = 0

for (i in hole_punch_row_vec) {
  for (j in hole_punch_col_vec) {
    if (num_hole_punch >= 750) {
      break
    } else {
      mat[i, j] = NA
      num_hole_punch = num_hole_punch + 1
    }
  }
  hole_punch_col_vec = sample(c(1:50), size = 50, replace = FALSE)
} 
